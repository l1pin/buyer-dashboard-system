...
export const offerStatusService = {
  ...
  // Получить историю статусов оффера
  async getOfferStatusHistory(offerId) {
    try {
      ...
      // Рассчитываем количество дней в каждом статусе
      const history = offerStatus.status_history.map((entry, index) => {
        ...
        let daysInStatus;
        if (index === 0) {
          // Для текущего статуса считаем до сегодняшнего дня
          daysInStatus = Math.floor((new Date() - changedAt) / (1000 * 60 * 60 * 24));
        } else if (nextEntry) {
          const nextChangedAt = new Date(nextEntry.changed_at);
          daysInStatus = Math.floor((changedAt - nextChangedAt) / (1000 * 60 * 60 * 24));
        } else {
          daysInStatus = 0;
        }
  ...
  // Удалить статус оффера (если нужно)
