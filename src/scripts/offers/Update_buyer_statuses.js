 ...
 * - "not_configured" (красный) - были активные дни в период доступа, но сегодня нет расхода
 * - "not_in_tracker" (фиолетовый) - нет активных дней в период доступа байера
 *
 * ВАЖНО: Учёт периодов доступа (access_granted/access_limited)
 * Один channel_id (source_id) может быть у разных байеров в разные периоды времени.
 * Статус определяется на основе данных ТОЛЬКО в период доступа конкретного байера.
 * Финальная корректировка статуса происходит в OfferBuyersPanel.js с учётом accessDatesMap.
 *
 * Для статусов 'not_configured' и 'not_in_tracker' в UI показывается
 * количество дней с момента привязки байера к офферу (created_at).
 *
 * ЛОГИКА:
 ...
 * 5. В UI корректируем статус с учётом периодов доступа (hasBuyerDataInAccessPeriod)
 */
