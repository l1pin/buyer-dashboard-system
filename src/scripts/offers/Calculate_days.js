/**
 * МАКСИМАЛЬНО ОПТИМИЗИРОВАННЫЙ скрипт для расчёта оставшихся дней продаж
 *
 * ПРОИЗВОДИТЕЛЬНОСТЬ (без лімітів Netlify):
 * – 🚀 Фильтрует по offer_id_tracker сразу в SQL (WHERE IN) - індекс працює ефективно
 * – 🚀 4 паралельних запити по 3 місяці (замість 12 по 1 місяцю) - менше HTTP overhead
 * – 🚀 Таймаут 60с (без обмеження Netlify 26с)
 * – 🚀 Швидкий fail-over з retry 1с
 *
 * ФУНКЦИОНАЛ:
 ...
 * – Використовує offer_id_tracker з БД API та маппінг article_offer_mapping
 */
...
async function fetchTrackerAll(offerIdArticleMap = {}) {
  ...
  // Створюємо SQL список для IN clause
  const offerIdsList = offerIds.map(id => `'${id.replace(/'/g, "''")}'`).join(',');

  // 🚀 МАКСИМАЛЬНА ОПТИМІЗАЦІЯ: 4 запити по 3 місяці (замість 12 по 1 місяцю)
  // Без лімітів Netlify можемо завантажувати більші обсяги за раз
  const periods = createQuarterlyPeriods(start, end);

  console.log(`📅 Завантаження ${periods.length} періодів (по 3 місяці) ПАРАЛЕЛЬНО...`);
  ...
  const promises = periods.map(async (p, i) => {
    ...
    console.log(`📦 [${i + 1}/${periods.length}] ${p.from}..${p.to} (3 місяці, паралельно)`);
  ...
  console.log(`✅ Завантажено ${all.length} записів за ${successCount}/${periods.length} періодів (по 3 місяці) - ПАРАЛЕЛЬНО 🚀`);
...
/**
 * 🚀 ОПТИМІЗАЦІЯ: Періоди по 3 місяці для мінімальної кількості запитів
 * 12 місяців = 4 запити (найшвидше без лімітів Netlify)
 */
function createQuarterlyPeriods(start, end) {
  ...
  while (cur <= end) {
    ...
    // +3 місяці (останній день 3-го місяця)
...
/**
 * Создаёт периоды по 1 месяцу (fallback)
 */
...
async function getDataBySql(strSQL, retryCount = 0) {
  const MAX_RETRIES = 2;      // 2 спроби
  const RETRY_DELAY = 1000;   // 1с між спробами (швидше)
  const FETCH_TIMEOUT = 60000; // 60 секунд (без ліміту Netlify 26с)
