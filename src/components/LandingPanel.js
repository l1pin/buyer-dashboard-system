...
></div>`;
    ...
    const handleCopyUuidCode = () => {
        const codeSnippet = `<div 
id="rt-meta" 
data-rt-sub16="${createdLandingUuid}"
></div>`;
    ...
    return (
        <div className="h-full flex flex-col bg-gray-50">
            ...
            {/* UUID Modal - Показ UUID после создания лендинга */}
            {showUuidModal && (
                <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
...
></div>`}
          ...
          <button
            ...
            className={`absolute top-3 right-3 px-3 py-2 rounded-md text-sm font-medium transition-all duration-200 ${
              copiedUuid ? 'bg-green-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
            }`}
            title="Копировать код"
          >
            {copiedUuid ? (
