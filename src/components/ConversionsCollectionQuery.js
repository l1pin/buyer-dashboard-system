// src/components/ConversionsCollectionQuery.js
// Конструктор SQL запросов для таблицы conversions_collection
...
// Доступные колонки таблицы conversions_collection
const AVAILABLE_COLUMNS = [
  ...
  // --- Дати ---
  { name: 'date_of_click', type: 'date' },
  { name: 'date_of_conversion', type: 'date' },
  ...
  { name: 'traffic_channel_id', type: 'string' },
  { name: 'offer_source', type: 'string' },
  { name: 'offer_source_id', type: 'string' },

  // --- SUB параметри ---
  { name: 'sub1', type: 'string' },
  { name: 'sub2', type: 'string' },
  { name: 'sub3', type: 'string' },
  { name: 'sub4', type: 'string' },
  { name: 'sub5', type: 'string' },
  { name: 'sub6', type: 'string' },
  { name: 'sub14', type: 'string' },
  { name: 'sub15', type: 'string' },
  { name: 'sub16', type: 'string' },
  { name: 'sub17', type: 'string' },
  { name: 'sub18', type: 'string' },
    ...
    <div ref={dropdownRef} className={`relative ${className}`}>
      <button
        ...
        className={`w-full px-3 py-2 text-left bg-white border rounded-lg text-sm flex items-center justify-between transition-colors ${
          isOpen ? 'border-amber-500 ring-2 ring-amber-100' : 'border-gray-300 hover:border-gray-400'
        }`}
      >
          ...
          <div className="p-2 border-b border-gray-100">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Поиск..."
                className="w-full pl-8 pr-3 py-1.5 text-sm border border-gray-200 rounded-md focus:outline-none focus:border-amber-400"
              />
            </div>
          </div>
...
function ConversionsCollectionQuery({ user }) {
  // Выбранные колонки
  const [selectedColumns, setSelectedColumns] = useState(['id', 'clickid', 'offer', 'offer_id', 'date_of_click', 'date_of_conversion', 'conv_type', 'source']);

  // Фильтры
  const [filters, setFilters] = useState([
    { column: 'date_of_click', condition: '>=', value: getDefaultDateFrom() }
  ]);
  ...
  const buildSqlQuery = useCallback(() => {
    ...
    let sql = `SELECT ${columns} FROM \`conversions_collection\``;
  ...
  const exportToCsv = () => {
    ...
    link.download = `conversions_collection_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
  ...
  const exportToXlsx = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'conversions_collection');
    XLSX.writeFile(wb, `conversions_collection_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
    ...
    <div className="p-6 max-w-full pb-12">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
          <Table2 className="w-7 h-7 text-amber-600" />
          SQL Query Builder
        </h1>
        <p className="text-gray-500 mt-1">Конструктор запросов к таблице conversions_collection</p>
      </div>
            ...
            <div className="max-h-[400px] overflow-y-auto space-y-1">
              {AVAILABLE_COLUMNS.map(col => (
                ...
                >
                  ...
                  <span className={`text-xs ml-auto ${col.type === 'date' || col.type === 'datetime' ? 'text-blue-400' : 'text-gray-400'}`}>
                    {col.type}
                  </span>
                </label>
              ))}
          ...
          {progress.show && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-amber-700">{progress.message}</span>
                <span className="text-sm text-amber-600">{progress.percent}%</span>
              </div>
...
export default ConversionsCollectionQuery;
