...
function SearcherCreativePanel({ user }) {
  ...
  const loadCreatives = async () => {
    try {
      ...
      for (const creative of searcherCreatives) {
      ...
      setCreatives(searcherCreatives);
    } catch (error) {
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Креативы, в которых вы указаны как Search Manager
            </p>
          </div>
        ...
        {creatives.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">
              Креативы будут отображаться здесь, когда монтажеры укажут вас как Search Manager
            </p>
          </div>
        ) : (
      ...
      {/* History Modal - аналогично BuyerCreativePanel */}
    </div>
  );
}

export default SearcherCreativePanel;
