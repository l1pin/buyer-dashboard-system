    ...
    const creativesSubscription = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          // 🔥 БЕЗОПАСНАЯ ЗАГРУЗКА МЕТРИК ДЛЯ НОВОГО КРЕАТИВА
          // Даем время хуку useBatchMetrics среагировать, потом проверяем и догружаем если нужно
          setTimeout(() => {
            console.log('⏳ Ждем автоматической загрузки метрик хуком...');
            
            // Еще одна проверка через 3 секунды - если метрики не загрузились, загружаем вручную
            setTimeout(async () => {
              try {
                // Проверяем, загрузились ли метрики для нового креатива
                const hasMetrics = getCreativeMetrics(payload.new.id)?.length > 0;
                
                if (!hasMetrics && payload.new.link_titles?.length > 0) {
                  console.log('⚠️ Метрики не загрузились автоматически, загружаем вручную для:', payload.new.article);
                  ...
                  console.log('✅ Метрики загружены вручную для нового креатива');
                } else {
                  console.log('✅ Метрики уже загружены автоматически для нового креатива');
                }
                
                // Обновляем зональные данные в любом случае
          ...
          }, 100); // Небольшая задержка перед запуском проверки
