    ...
    const creativesSubscription = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          // ⏳ ЖДЕМ ПОКА CreativePanel.js ЗАГРУЗИТ МЕТРИКИ В БД, ПОТОМ ЗАГРУЖАЕМ ИХ ЗДЕСЬ
          console.log('⏳ Ждем 5 секунды, пока CreativePanel.js загрузит метрики для нового креатива...');
          setTimeout(async () => {
            try {
              console.log('🔄 Обновляем метрики и зоны для нового креатива:', payload.new.article);
              
              // Просто обновляем ВСЕ метрики - хук сам подтянет новые, не сбросив старые
              await refreshMetrics();
              await refreshZoneData();
              
              console.log('✅ Метрики и зональные данные обновлены');
            } catch (error) {
              console.error('❌ Ошибка обновления метрик:', error);
            }
          }, 5000); // Ждем 5 секунды - CreativePanel.js успеет загрузить и сохранить метрики
