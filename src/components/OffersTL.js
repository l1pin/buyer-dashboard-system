  ...
  const autoUpdateMetrics = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        // Сохраняем данные по source_id для метрик байеров (за ВСЕ 12 месяцев!)
  ...
  const updateLeadsData = async () => {
    try {
      ...
      if (result.dataBySourceIdAndDate) {
        ...
        console.log(`📊 Сохранены метрики по ${Object.keys(result.dataBySourceIdAndDate).length} артикулам`);
      }
