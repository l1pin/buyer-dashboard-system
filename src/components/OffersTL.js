...
function OffersTL({ user }) {
  ...
  const autoUpdateMetrics = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        // Сохраняем данные по source_id для метрик байеров (за ВСЕ 12 месяцев из preloadedData!)
  ...
  const updateLeadsData = async () => {
    try {
      ...
      // 🎯 Используем result.dataBySourceIdAndDate для метрик байеров (данные за 12 месяцев)
      const result = await updateLeadsFromSqlScript(metrics, articleOfferMap);

      setMetrics(result.metrics);

      // 🎯 Используем данные из result.dataBySourceIdAndDate для метрик байеров
      setBuyerMetricsData(result.dataBySourceIdAndDate);
      console.log(`📊 Метрики байеров загружены (${Object.keys(result.dataBySourceIdAndDate || {}).length} офферов)`);
