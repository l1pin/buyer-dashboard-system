...
function OffersTL({ user }) {
  ...
  const autoUpdateMetrics = useCallback(async () => {
    ...
    try {
      ...
      // 🎯 Запускаем загрузку метрик байеров за ВСЁ ВРЕМЯ параллельно (без ограничения по датам!)
      ...
      try {
        ...
        // 🎯 Ждём завершения загрузки метрик байеров за ВСЁ время (без ограничения по датам!)
        const buyerMetricsAllTime = await buyerMetricsPromise;
        setBuyerMetricsData(buyerMetricsAllTime);
        console.log(`✅ Метрики байеров загружены за ВСЁ время`);
