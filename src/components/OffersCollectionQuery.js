// src/components/OffersCollectionQuery.js
// Конструктор SQL запросов для таблицы offers_collection
...
// Доступные колонки таблицы offers_collection
const AVAILABLE_COLUMNS = [
  // --- Ідентифікація ---
  { name: 'id', type: 'number' },
  { name: 'salesdrive_sku', type: 'string' },
  { name: 'offer_name', type: 'string' },
  { name: 'category', type: 'string' },
  { name: 'redtrack_id', type: 'string' },

  // --- Статистика (Апрув та Викуп) ---
  { name: 'approve_percent_oper', type: 'number' },
  { name: 'sold_percent_oper', type: 'number' },
  { name: 'approve_percent_manual', type: 'number' },
  { name: 'sold_percent_manual', type: 'number' },

  // --- Фінанси (Середньозважені ціни) ---
  { name: 'av_offer_base_price', type: 'number' },
  { name: 'av_offer_invest_price', type: 'number' },
  { name: 'av_offer_sell_price', type: 'number' },
  { name: 'av_delivery_cost', type: 'number' },

  // --- Допродажі (Upsell) ---
  { name: 'av_connected_base_price', type: 'number' },
  { name: 'av_connected_sell_price', type: 'number' },

  // --- Зони ефективності та Аналітика (JSON поля) ---
  { name: 'effectivity_zone', type: 'json' },
  { name: 'last_result_conversions', type: 'json' },
  { name: 'arr_modifications', type: 'json' },
  { name: 'zone_history', type: 'json' },

  // --- Службові ---
  { name: 'approve_percent_final', type: 'number' },
  { name: 'sold_percent_final', type: 'number' },
  { name: 'connected_buyers', type: 'json' },
  { name: 'adv_media_items', type: 'json' }
];
...
function OffersCollectionQuery({ user }) {
  // Выбранные колонки
  const [selectedColumns, setSelectedColumns] = useState(['id', 'offer_name', 'category', 'redtrack_id', 'approve_percent_oper', 'av_offer_invest_price']);

  // Фильтры
  const [filters, setFilters] = useState([
    { column: 'offer_name', condition: 'IS NOT NULL', value: '' }
  ]);
  ...
  const buildSqlQuery = useCallback(() => {
    ...
    let sql = `SELECT ${columns} FROM \`offers_collection\``;
  ...
  const exportToCsv = () => {
    ...
    link.download = `offers_collection_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
  ...
  const exportToXlsx = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'offers_collection');

    // Скачиваем файл
    XLSX.writeFile(wb, `offers_collection_${new Date().toISOString().split('T')[0]}.xlsx`);
  };

  // Форматирование значений JSON для отображения
  const formatCellValue = (cell, header) => {
    if (cell === null || cell === undefined) {
      return <span className="text-gray-300 italic">null</span>;
    }

    // Если это JSON поле, форматируем красиво
    const jsonColumns = ['effectivity_zone', 'last_result_conversions', 'arr_modifications', 'zone_history', 'connected_buyers', 'adv_media_items'];
  ...
  return (
    <div className="p-6 max-w-full pb-12">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">Конструктор запросов к таблице offers_collection</p>
      </div>
            ...
            <div className="max-h-[400px] overflow-y-auto space-y-1">
              {AVAILABLE_COLUMNS.map(col => (
                ...
                >
                  ...
                  <span className={`text-xs ml-auto ${col.type === 'json' ? 'text-purple-400' : 'text-gray-400'}`}>{col.type}</span>
                </label>
              ))}
            </div>
          ...
          {progress.show && (
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-emerald-700">{progress.message}</span>
...
export default OffersCollectionQuery;
