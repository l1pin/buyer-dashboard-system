// src/components/SalesCollectionQuery.js
// Конструктор SQL запросов для таблицы sales_collection
...
// Доступные колонки таблицы sales_collection
const AVAILABLE_COLUMNS = [
  ...
  { name: 'delivery_payer', type: 'string' },
  { name: 'delivery_price', type: 'number' },

  // --- Замовлені товари ---
  { name: 'ordered_list', type: 'json' },

  // --- Фінанси замовлення ---
  { name: 'order_base_price', type: 'number' },
  { name: 'order_price', type: 'number' },
  { name: 'order_end_price', type: 'number' },
  { name: 'order_profit', type: 'number' },
  { name: 'order_aditional_sell', type: 'number' },

  // --- Початкові дані кліку ---
  { name: 'initial_sku', type: 'string' },
  { name: 'initial_sub_sku', type: 'string' },
  { name: 'initial_offer_name', type: 'string' },
  { name: 'initial_offer_id', type: 'string' },
  { name: 'initial_base_price', type: 'number' },
  { name: 'initial_price', type: 'number' },
  { name: 'initial_traffic_channel_id', type: 'string' },
    ...
    <div ref={dropdownRef} className={`relative ${className}`}>
      <button
        ...
        className={`w-full px-3 py-2 text-left bg-white border rounded-lg text-sm flex items-center justify-between transition-colors ${
          isOpen ? 'border-violet-500 ring-2 ring-violet-100' : 'border-gray-300 hover:border-gray-400'
        }`}
      >
          ...
          <div className="p-2 border-b border-gray-100">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Поиск..."
                className="w-full pl-8 pr-3 py-1.5 text-sm border border-gray-200 rounded-md focus:outline-none focus:border-violet-400"
              />
            </div>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      {optType && <span className={`text-xs ${optType === 'json' ? 'text-purple-400' : optType === 'datetime' ? 'text-blue-400' : 'text-gray-400'}`}>{optType}</span>}
...
// Кастомный дропдаун для условий
...
function SalesCollectionQuery({ user }) {
  // Выбранные колонки
  const [selectedColumns, setSelectedColumns] = useState([
    'id', 'clickid', 'order_id', 'order_status', 'order_date',
    'order_price', 'order_end_price', 'order_profit', 'initial_offer_name'
  ]);

  // Фильтры
  const [filters, setFilters] = useState([
    { column: 'order_date', condition: '>=', value: getDefaultDateFrom() }
  ]);
  ...
  const buildSqlQuery = useCallback(() => {
    ...
    let sql = `SELECT ${columns} FROM \`sales_collection\``;
  ...
  const exportToCsv = () => {
    ...
    link.download = `sales_collection_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
  ...
  const exportToXlsx = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'sales_collection');
    XLSX.writeFile(wb, `sales_collection_${new Date().toISOString().split('T')[0]}.xlsx`);
  };

  // Форматирование JSON полей
  const formatCellValue = (cell, header) => {
    if (cell === null || cell === undefined) {
      return <span className="text-gray-300 italic">null</span>;
    }

    if (header === 'ordered_list') {
      try {
        const parsed = typeof cell === 'string' ? JSON.parse(cell) : cell;
        return (
          <span className="text-xs font-mono text-purple-600 max-w-xs truncate block" title={JSON.stringify(parsed, null, 2)}>
            {JSON.stringify(parsed).slice(0, 50)}{JSON.stringify(parsed).length > 50 ? '...' : ''}
          </span>
        );
      } catch {
        return String(cell);
    ...
    const moneyColumns = ['delivery_price', 'order_base_price', 'order_price', 'order_end_price', 'order_profit', 'order_aditional_sell', 'initial_base_price', 'initial_price'];
    if (moneyColumns.includes(header)) {
      const num = parseFloat(cell);
      if (!isNaN(num)) {
        return <span className={num < 0 ? 'text-red-600' : 'text-green-600'}>{num.toFixed(2)}</span>;
      }
    }
    ...
    <div className="p-6 max-w-full pb-12">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
          <Table2 className="w-7 h-7 text-violet-600" />
          SQL Query Builder
        </h1>
        <p className="text-gray-500 mt-1">Конструктор запросов к таблице sales_collection (CRM продажі)</p>
      </div>
              ...
              {AVAILABLE_COLUMNS.map(col => (
                ...
                >
                  ...
                  <span className={`text-xs ml-auto ${
                    col.type === 'json' ? 'text-purple-400' :
                    col.type === 'datetime' ? 'text-blue-400' :
                    col.type === 'number' ? 'text-green-400' :
                    'text-gray-400'
                  }`}>
                    {col.type}
                  </span>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
            ...
            <div className="mt-4 pt-4 border-t flex items-center gap-4">
              ...
              <input
                ...
                className="w-32 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-violet-500 focus:border-violet-500"
                min="0"
                max="100000"
              />
            </div>
          ...
          {progress.show && (
            <div className="bg-violet-50 border border-violet-200 rounded-xl p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-violet-700">{progress.message}</span>
                <span className="text-sm text-violet-600">{progress.percent}%</span>
              </div>
                  ...
                  <tr key={rowIndex} className="hover:bg-gray-50">
                    ...
                    {row.map((cell, cellIndex) => (
                      <td key={cellIndex} className="px-3 py-2 text-gray-700 whitespace-nowrap">
                        {formatCellValue(cell, results.headers[cellIndex])}
                      </td>
                    ))}
                  </tr>
                ))}
...
export default SalesCollectionQuery;
