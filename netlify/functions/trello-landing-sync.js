...
exports.handler = async (event, context) => {
  ...
  try {
    const { landingId, trelloLink, isTest } = JSON.parse(event.body || '{}');

    if (!landingId || !trelloLink) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({
          error: 'Missing required parameters',
          message: 'landingId and trelloLink are required'
        })
      };
    }

    const boardType = isTest ? 'test' : 'main';
    console.log('🔄 Синхронизация Trello карточки для лендинга:', landingId);
    console.log('🔗 Trello link:', trelloLink);
    console.log('📋 Board type:', boardType);

    // Извлекаем короткий ID из URL
    ...
    if (!cardResponse.ok) {
      ...
      console.error('❌ Ошибка API Trello:', cardResponse.status, errorText);
